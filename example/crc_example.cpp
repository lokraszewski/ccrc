...
// template <typename T>
// void checksum_and_print_(std::string label, T &crc, const uint8_t *buffer,
// const size_t size)
// {
//   auto       start = std::chrono::steady_clock::now();
//   const auto res   = crc.process_bytes(buffer, size);
//   auto       end   = std::chrono::steady_clock::now();
//   l_log->info("{:>16} : {:#x} : {}ns", label, res,
//   std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count());
// }

// #define checksum_and_print(name, buffer, size) checksum_and_print_(#name,
// (name), buffer, size)
